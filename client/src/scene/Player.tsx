...
export function Player() {
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (useGameStore.getState().isPaintDragging) {
        // Đang vẽ: camera ĐỨNG YÊN, chuột di chuyển con trỏ vẽ tự do trong
        // khung hình (giống app vẽ thật) — không xoay camera lúc này.
        const { reticleOffset, setReticleOffset } = useGameStore.getState();
        const ndcDeltaX = e.movementX / (window.innerWidth / 2);
        const ndcDeltaY = -e.movementY / (window.innerHeight / 2);
        setReticleOffset({
          x: THREE.MathUtils.clamp(reticleOffset.x + ndcDeltaX, -0.85, 0.85),
          y: THREE.MathUtils.clamp(reticleOffset.y + ndcDeltaY, -0.85, 0.85),
        });
        return;
      }
  ...
  useFrame(() => {
    ...
    // Nếu KHÔNG rơi vào 3 trường hợp trên (Hider đứng yên, third-person):
    // camera orbit tự do quanh người mà KHÔNG làm người quay theo — đây là
    // điều kiện để đứng yên xoay camera ngắm các mặt khác của cơ thể để vẽ.
    const isMoving =
    ...
    if (isFirstPerson.current) {
      ...
      camera.position.set(camX, pos.y - 0.1 + verticalOffset, camZ);
      camera.lookAt(pos.x, pos.y - 0.1, pos.z);
    }

    sendLocalTransform(pos.x, pos.y, pos.z, facingYaw.current);
