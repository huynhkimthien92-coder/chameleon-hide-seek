...
export function Player() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Escape" && useGameStore.getState().isPaintBoardOpen) {
        useGameStore.getState().setIsPaintBoardOpen(false);
      }
    };
  ...
  useFrame(() => {
    ...
    // Bảng vẽ đang mở -> tạm dừng di chuyển (người chơi đang tập trung vẽ,
    // không nên để nhân vật trôi đi vì WASD vẫn nhận input nền).
    const isPaintBoardOpen = useGameStore.getState().isPaintBoardOpen;
