...
## Vẽ tự do — ĐỔI SANG Bảng vẽ 2D (thay cách vẽ trong không gian 3D)

Sau nhiều lần sửa (orbit camera, tách facingYaw, reticle con trỏ tự do...) vẫn không giải quyết được gốc vấn đề: **vừa phải điều khiển camera 3D vừa phải nhắm trúng điểm chính xác trên 1 mô hình nhỏ là việc cố hữu khó kiểm soát**, không hợp cho việc vẽ. Đã đổi hẳn sang **Bảng vẽ 2D** (`ui/PaintBoard.tsx`):

- Hút màu từ môi trường vẫn làm trong 3D như cũ (không đổi).
- Bấm nút **"Bảng vẽ"** (cạnh swatch màu) → mở overlay hiện 4 ô vuông đầu/thân/tay/chân.
- **Vẽ trực tiếp bằng cách kéo chuột trên 4 ô đó** — y như Paint/Photoshop, không còn camera, không còn nhắm trúng/hụt.
- Đóng bảng (nút X hoặc phím Escape) → quay lại chơi bình thường, thấy ngay kết quả lên người trong 3D.
- Cơ chế canvas/texture/đồng bộ mạng (`paintRegistry.ts`, message `paintStroke`) giữ nguyên 100% — chỉ đổi NGUỒN kích hoạt nét vẽ (chuột trên canvas 2D thay vì raycast 3D), nên không cần đổi gì ở server/đồng bộ.
- Đã dọn bỏ toàn bộ code cũ không còn dùng: orbit pitch cho mục đích vẽ, reticle, `isAimingOwnBody`, `userData.isBodyPart` trên mannequin. Phần tách `facingYaw`/camera `yaw` (sửa "người xoay theo camera") vẫn giữ — vẫn có giá trị độc lập (đứng yên xoay camera ngắm bản thân không bị xoay người theo), không liên quan tới việc vẽ nữa nhưng không hại gì khi giữ lại.
- Trong lúc Bảng vẽ mở: WASD tạm dừng (tránh nhân vật trôi đi), chuột được nhả khỏi pointer-lock để tương tác UI 2D bình thường.

## Bug đã sửa từ test thật trên bản deploy

✅ **"Xoay tầm nhìn thì người cũng xoay theo, không bao giờ đứng yên để vẽ được"** — đúng, nguyên nhân: hướng nhân vật (mannequin quay mặt đi đâu) trước đó **gắn cứng theo góc camera (yaw)** — mọi lần ngoáy chuột để ngắm mặt khác của cơ thể, nhân vật lập tức quay theo, không bao giờ "đứng yên cho camera đi vòng quanh". Đã tách riêng: **hướng camera nhìn (yaw, luôn theo chuột)** và **hướng nhân vật thật (facingYaw, chỉ đổi khi đang bấm WASD di chuyển)**. Hệ quả: Hider đứng yên (không bấm di chuyển) ở third-person giờ xoay chuột chỉ xoay camera quanh người, người không quay theo — đủ để ngắm/vẽ mọi mặt cơ thể. Seeker và First-person không đổi gì (vẫn cần camera = hướng ngắm chính xác mọi lúc để bắn đúng).
...
- **Nếu là Hider**: nhìn vào bề mặt môi trường rồi **click** để hút màu (cầm sẵn) → bấm nút **"Bảng vẽ"** → overlay 2D hiện ra, **kéo chuột trên 4 ô đầu/thân/tay/chân** để vẽ như Paint thật → đóng bảng (X hoặc Escape) để thấy kết quả lên người trong 3D. Toolbar dưới màn hình còn có dãy nút đổi pose (kể cả "Đóng băng").
