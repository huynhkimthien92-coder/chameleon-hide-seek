...
// Cho phép override bằng env khi test (smoke test không muốn chờ 5s mỗi lần) —
// production vẫn mặc định 5000ms.
...
export class GameRoom extends Room<{ state: GameState }> {
  ...
  private tryStartMatch() {
    ...
    this.matchTimer = this.clock.setInterval(() => this.tickMatch(), 1000);
    console.log(`[GameRoom] Trận bắt đầu — ${this.state.players.size} người chơi`);
  }
